...
- 连接 MySQL 服务器并浏览数据库、表和视图；连接权限、对象树和表记录数会在本地服务存活期间缓存。可记住登录信息，并对已保存连接进行编辑或删除；打开连接对话框时会刷新已保存连接列表，对话框内就地显示成功/失败提示。
- 分页查看表数据，支持最多 8 个筛选条件组合（AND / OR）及最多 8 个字段的升降序排序；可复制整条记录（自增主键会自动跳过），并批量删除选中记录。行内「操作」列默认收起，可按需展开。
